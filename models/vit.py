...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None, return_prompt_sims=False):
            ...
            if prompt is not None:
                if train:
                    ...
                    if return_prompt_sims:
                        p_list, _, x, sim_info = prompt.forward(q, i, x, train=False, task_id=task_id, return_sim=True)
                        if sim_info is not None:
                            prompt_sims[i] = sim_info
                    else:
        ...
        if return_prompt_sims:
            return x, prompt_loss, prompt_sims
        else:
